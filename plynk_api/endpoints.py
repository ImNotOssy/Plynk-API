""" All the URLs we will use to interact with Plynk """

def digital_url() -> str:
    return "https://www.digitalbrokerageservices.com"


def ecaap_url() -> str:
    return "https://ecaap.digitalbrokerageservices.com"


def authentication_url() -> str:
    return f"{ecaap_url()}/user/factor/password/authentication"


def login_url() -> str:
    return f"{ecaap_url()}/user/session/login"


def phone_url() -> str:
    return f"{ecaap_url()}/user/identity/attributes?attributes=phone"


def sms_url() -> str:
    return f"{ecaap_url()}/user/factor/otp/sms/authentication"

def details_url() -> str:
    return f"{digital_url()}/gateway/restrict/accounts/v1/details"

def account_url() -> str:
    return f"{digital_url()}/gateway/restrict/portfolio/v1/customer/accounts"


def market_open_url() -> str:
    return f"{digital_url()}/gateway/restrict/gp-market-calendar/v2/markets/reference/business-dates"


def balance_url() -> str:
    return f"{digital_url()}/gateway/restrict/accounts/v1/accounts/balance"


def positions_url() -> str:
    return f"{digital_url()}/gateway/restrict/accounts/v1/accounts/positions"


def stock_search_url(ticker: str) -> str:
    return f"{digital_url()}/gateway/restrict/market-data/v2/securities/lookup?highRiskETFsEnabled=true&input={ticker.lower()}"


def stock_details_url() -> str:
    return f"{digital_url()}/gateway/restrict/market-data/v2/securities/details"


def place_order_url() -> str:
    return f"{digital_url()}/gateway/restrict/brokerage-order-entry/v5/accounts/orders/equities"


def build_headers(domain: str, login_poll: bool = False) -> dict:
    """
    Builds specific headers for each domain and request type based on successful curl commands.
    :param domain: Either 'ecaap' for login or 'digital' for data.
    :param login_poll: Set to True for the specific session/login polling step.
    """
    base_headers = {
        "Accept": "application/json",
        "Appname": "Simplifid",
        "Accept-Token-Location": "HEADER",
        "Accept-Language": "en-US,en;q=0.5",
        "Accept-Encoding": "gzip, deflate, br",
        "User-Agent": "Plynk/1842 CFNetwork/1492.0.1 Darwin/23.3.0",
        "Token-Location": "HEADER",
        "Accept-Token-Type": "ET",
        "Connection": "keep-alive",
        "Content-Type": "application/json",
        "x-acf-sensor-data": "4,i,qMG9W1OFbObs5300s4c3USRKhJatHNMKfzxKSn2GLJYoGDjzqEmpZelg/6F6Vqo7O6K+c9g6WohHAr/oomyQL9Rhd/qk7xhIiCOXFGTT5WPIy3OVdSzLOgO5mTv3BkgXZ/JG6KMD0opGB/7IpYaG+lDCF2vZW486ASbJXfqEyH0=,GxzDvgnmfXikqi6fkl7Xcsy7wtOOVV4dGmvAc/TG0xGyvcCFVOXXSBZgsZxyhiz0xA7C0yKRv011fnpETUfIoCvd2bSnNHITi86Rd/ojNt9CMidFEEAD7vwOtc67CdB1SaRnn4IjBPwrc4qXWEFsFrxTWpiDkGu0M5gWLOAIbB8=$$28,17,51",
    }

    if domain == 'ecaap':
        base_headers['Host'] = 'ecaap.digitalbrokerageservices.com'
        base_headers['Appid'] = 'DBS-SIMPLIFID'
        if login_poll:
            base_headers['Eventtype'] = 'LOGIN'
            base_headers['Sub_eventtype'] = 'login_analyze'
    elif domain == 'digital':
        base_headers['Host'] = 'www.digitalbrokerageservices.com'
        base_headers['Appid'] = 'AP128920'
    else:
        raise ValueError("Domain must be 'ecaap' or 'digital'")
        
    return base_headers
